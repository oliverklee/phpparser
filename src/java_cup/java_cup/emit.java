...
public class emit {
  ...
  public static void symbols(PrintWriter out, 
      ...
      for (Enumeration e = terminal.all(); e.hasMoreElements(); )
	{
      ...
      // NJ:
      // we don't want the nonterminals' indices to overlap with the terminals'
      // indices, so we start with the largest terminal index for the nonterminals
      int baseIndex = terminal.number();
      ...
      if (emit_non_terms)
	{
          ...
          for (Enumeration e = non_terminal.all(); e.hasMoreElements(); )
	    {
	      ...
	      out.println("  static final int " + nt.name() + " = " + 
		          nt.index() + ";");
          */
	      out.println("  public static final int " + nt.name() + " = " + 
		          (baseIndex + nt.index()) + ";");
	    }
	}
  ...
  protected static void emit_action_code(PrintWriter out, production start_prod)
      ...
      if (action_code != null)
	{
      ...
      // NJ:
      // field containing the number of terminals
      out.println("  private final int numTerminals = " + terminal.number() + ";");
      ...
      for (Enumeration p = production.all(); p.hasMoreElements(); )
	{
      ...
      /* NJ: make production number and name accessible  */
      out.println("               int prodNumber = " + prod.lhs().the_symbol().index() + ";");
      out.println("               String prodName = \"" + prod.lhs().the_symbol().name() + "\";");
      out.println("               int prodLength = " + prod.rhs_length() + ";");
